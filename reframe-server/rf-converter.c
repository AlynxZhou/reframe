...
static int _setup_gl(RfConverter *this)
{
	if (!eglMakeCurrent(
		...
		g_warning("EGL: Failed to make context current: %d.",
			  eglGetError());
		return -4;
	}
	...
	for (unsigned int rotation = this->rotation; rotation != 0; rotation -= 90) {
		float u = coordinates[6];
		float v = coordinates[7];
